...
func init() {
	...
	http.HandleFunc("/setEnergyLevel", func(rw http.ResponseWriter, r *http.Request) {
		energyLevel := r.URL.RawQuery
		energyLevelInt, err := strconv.Atoi(energyLevel)
		chkPrint(err)
		fftColorBufferSize = energyLevelInt
	})
	http.HandleFunc("/getVariables", func(rw http.ResponseWriter, r *http.Request) {
	...
	http.HandleFunc("/setColorShift", func(rw http.ResponseWriter, r *http.Request) {
		energyLevel := r.URL.RawQuery
		energyLevelInt, err := strconv.Atoi(energyLevel)
		chkPrint(err)
		fftColorShift = float64(energyLevelInt)
	})
	http.HandleFunc("/setColorBrightness", func(rw http.ResponseWriter, r *http.Request) {
		energyLevel := r.URL.RawQuery
		energyLevelInt, err := strconv.Atoi(energyLevel)
		chkPrint(err)
