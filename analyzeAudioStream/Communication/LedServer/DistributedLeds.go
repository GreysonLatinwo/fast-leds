...
func main() {
	...
	webServerPortFlag := flag.Int("webport", 9001, "Port for the webserver to listen on")
	flag.Parse()

	joinRoom(*nickFlag, *roomFlag)

	log.Println("Hosting on", WebServerAddr+":"+fmt.Sprint(*webServerPortFlag))
	log.Println("Self: " + chatRoom.nick)
	go http.ListenAndServe(WebServerAddr+":"+fmt.Sprint(*webServerPortFlag), nil)

	color := [3]uint8{0, 0, 0}
	go AnalyzeAudio(&color)
	readColorLoop(&color)
}

//takes the color output and tells the network
func readColorLoop(color *[3]uint8) {
	for {
		chatRoom.Publish([]byte{color[0], color[1], color[2]})
...
func joinRoom(nickname string, room string) {
	...
	log.Println("Discovery Service Tag:", DiscoveryServiceTag)
